  ...
  const handleGenerate = useCallback(
    async (
      ...
      try {
        ...
        console.error(err);
        setError(
          "Failed to compose a new piece. Please check your connection and try again.",
        );
              ...
              {history.length === 0 ? (
                ...
                <ul className="space-y-3">
                  {history.map((item, idx) => (
                    <li
                      key={idx}
                      className="text-sm text-stone-600 flex justify-between items-center"
                    >
                      ...
                      <div className="flex gap-1">
                        ...
                        <span className="text-xs bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded font-semibold">
                          Lvl {item.difficulty}
                        </span>
                      </div>
                    </li>
          ...
          <div className="lg:col-span-9">
            <div className="soft-surface min-h-[600px] flex flex-col relative overflow-hidden border border-black/5">
              ...
              <div className="flex-grow p-4 sm:p-8 flex flex-col items-center justify-start overflow-auto music-container bg-transparent relative">
                ...
                {!exercise && loadingState === "idle" && !error && (
                  <div className="flex flex-col items-center justify-center h-full text-slate-300 relative z-10">
                    <svg
                      className="w-24 h-24 mb-6 opacity-30 drop-shadow-sm"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                    ...
                    <p className="font-heading text-xl text-slate-400">
                      Music notation will appear here
                    </p>
                  </div>
                )}
              </div>
              ...
              {exercise && (
                <div
                  id="audio-controls"
                  className="p-4 border-t border-[#E8DEC1] bg-[#FDFBF7]/80 backdrop-blur-sm flex justify-center z-10 relative"
                >
