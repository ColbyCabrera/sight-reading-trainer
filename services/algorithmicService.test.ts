...
import type { DifficultyLevel, MusicalKey, GenerationSettings } from '../types.ts';
...
describe('generateAlgorithmicSheetMusic', () => {
  test('should generate a valid exercise for difficulty level 1', () => {
    ...
    assert.ok(result.title.includes('C Major') || result.title.includes('C') || result.title.includes('Cm'));
  ...
  test('should generate a valid exercise for high difficulty (level 8)', () => {
    ...
    assert.ok(result.abcNotation.includes('M:3/4') || result.abcNotation.includes('M:4/4'));
  });
  ...
  test('should handle "Random" key', () => {
    ...
    assert.ok(result.abcNotation.includes(`K:${result.key.replace(' Major', '').replace(' Minor', 'm')}`));
  });
